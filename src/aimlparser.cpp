...
bool exactMatch(QString regExp, QString str, QStringList &capturedText)
{
    ...
    if ((!regExpWords.count() || !strWords.count()) &&
                                (regExpWords.count() != strWords.count()))
        return false;
    if (regExpWords.count() > strWords.count())
        return false;
    ...
    QStringList &capturedTopicTexts, Leaf *&leaf)
{
    ...
    if ((word == "*") || (word == "_"))
    {
        ...
        for (;input != inputWords.end(); input++)
        {
            ...
            {
                if (child->match(input, inputWords, currentThat, currentTopic,
    ...
    if (input == inputWords.end())
    {
        foreach (leaf, leafs)
        {
            ...
            if ( (!leaf->that.isEmpty() &&
                    !exactMatch(leaf->that, currentThat, capturedThatTexts)) ||
                    (!leaf->topic.isEmpty() && 
...
// Modified for graphviz
/* void Node::debug(QTextStream* dotStream, uint indent)
{
    QString indentStr(" -> ");
    if (!word.isEmpty()) {
	if (indent > 1)
	        (*dotStream) << indentStr << "\"" << indent << "." << word << "\"";
	else if (indent == 1)
	        (*dotStream) << "_input -> " << "\"" << indent << "." << word << "\"";
    }

    // Do next word
    foreach (Node* child, childs)
        child->debug(dotStream, indent+1);

    // Finish with that and topic
    foreach (Leaf* leaf, leafs) {
        if (leaf->that != "") {
            (*dotStream) << indentStr;
            (*dotStream) << " -> that_" << leaf->that;
        }
        if (leaf->topic != "") {
            (*dotStream) << " -> topic_" << leaf->topic;
        }
        (*dotStream) << "\n";
    }
...
{
    QDomDocument doc;
    QFile file( currentPath + "/aiml_set/init/TestSuite.xml" );
    ...
    loadAiml(currentPath + "/aiml_set/init/TestSuite.aiml");
...
AIMLParser::AIMLParser(bool displayTree) : displayTree(displayTree)
{
    ...
    tcpServer = NULL;
    //load init files
    loadVars(currentPath + "/aiml_set/init/vars.xml", false);
    loadSubstitutions(currentPath + "/aiml_set/init/substitutions.xml");
}

AIMLParser::~AIMLParser()
{
//    saveVars(currentPath + "/aiml_set/init/vars.xml");
    delete logStream;
}
...
bool AIMLParser::loadAIMLSet(const QString &aimlSet)
{
    ...
    for (QStringList::Iterator it = files.begin(); it != files.end(); ++it)
        loadAiml(dirname + "/" + *it);
    if (displayTree) {
        //Create the graphviz dot file
        QFile *dotfile = new QFile("grammar.dot", this);
        dotfile->open(QIODevice::WriteOnly);
        QTextStream* dotStream = new QTextStream(dotfile);
        dotStream->setCodec(QTextCodec::codecForName("UTF-8"));
        (*dotStream) << "digraph tree\n{\nnodesep=0.5\nrankdir=LR\nfixedsize=true\nconcentrate=true\nnode [style=\"rounded,filled\", width=0, height=0, shape=box]\n\n";
        root.debug(dotStream);
	(*dotStream) << "}";
        return false;
    }
...
bool AIMLParser::loadSubstitutions(const QString &filename)
{
    ...
    QFile file( filename );
    if ( !file.open( QIODevice::ReadOnly ) ) {
        (*logStream) << QString("Error opening %1\n").arg(qPrintable(filename));
        return false;
    }
    if ( !doc.setContent( &file ) )
    {
...
void AIMLParser::setVar(const QString &name, const QString &value)
{
    parameterValue[name] = value;
}
...
void AIMLParser::parseCategory(QDomNode* categoryNode)
{
    ...
    {
        int childLeafWeight = !childLeaf->that.isEmpty() +
                                !childLeaf->topic.isEmpty() * 2;
                                ...
                                const QStringList &capturedThatTexts, const QStringList &capturedTopicTexts)
{
    ...
    if (nodeName == "random")
    {
        ...
        result = resolveNode(&child, capturedTexts, capturedThatTexts,
                            capturedTopicTexts);
    }
    else if (nodeName == "condition")
    {
        ...
        if (element.hasAttribute("name"))
        {
            ...
            if (element.hasAttribute("value"))
            {
                ...
                {
                    (*logStream) << "::Matched condition: " + name + "[" + value + "]\n";
                    logStream->flush();
                    //dirty trick to avoid infinite loop !
                    element.setTagName("parsedCondition");
                    result = resolveNode(&element, capturedTexts,
                                        capturedThatTexts, capturedTopicTexts);
        ...
        if (condType)
        {
            ...
            for (int i = 0; i < childNodes.count(); i++)
            {
                ...
                if (n.toElement().hasAttribute("value"))
                {
                    ...
                    if (exactMatch(value, parameterValue[name].toUpper(),dummy))
                    {
                        (*logStream) << "::Matched li: " + name + "[" + value + "]\n";
                        logStream->flush();
                        result = resolveNode(&n, capturedTexts,
                                        capturedThatTexts, capturedTopicTexts);
                        break;
                    }
                }
                ...
                {
                    result = resolveNode(&n, capturedTexts, capturedThatTexts,
                                            capturedTopicTexts);
                    break;
                }
            }
    ...
    else
    {
        ...
        else if (!node->hasChildNodes())
        {
            if (nodeName == "star")
            {
                ...
                result = index < capturedTexts.count() ?
                                 capturedTexts[index] : QString("");
            }
            else if (nodeName == "thatstar")
            {
                uint index = element.attribute("index", "1").toUInt() - 1;
                result = index < capturedThatTexts.count() ?
                                 capturedThatTexts[index] : QString("");
            }
            else if (nodeName == "topicstar")
            {
                uint index = element.attribute("index", "1").toUInt() - 1;
                result = index < capturedTopicTexts.count() ?
                                 capturedTopicTexts[index] : QString("");
            }
            else if (nodeName == "that")
            {
                ...
                result = (index1 < thatList.count()) &&
                        (index2 < thatList[index1].count()) ?
                         thatList[index1][index2] : QString("");
            }
            else if (nodeName == "sr")
                result = getResponse(capturedTexts.count() ?
                                 capturedTexts[0] : QString(""), true);
            else if (nodeName == "br")
                result = "\n";
            ...
            else if ( (nodeName == "person") || (nodeName == "person2") ||
                      (nodeName == "gender") )
                result = capturedTexts.count() ? capturedTexts[0] : QString("");
            else if (nodeName == "input")
            {
                uint index = element.attribute("index", "1").toUInt() - 1;
                result = index < inputList.count() ?
                                 inputList[index] : QString("");
            }
        ...
        //the following just to avoid warnings !
        else if ((nodeName == "template") || (nodeName == "pattern") ||
                 (nodeName == "li") || (nodeName == "person") ||
                 (nodeName == "person2") || (nodeName == "gender")
...
QString AIMLParser::getResponse(QString input, const bool &srai)
{
    ...
    if (!srai)
    {
        ...
        //get the sentences of the result splitted by: . ? ! ;
        QStringList thatSentencesList = tempResult.split(QRegExp("[\\.\\?!;]"), QString::SkipEmptyParts);
